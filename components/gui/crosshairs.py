...
def getAllCrosshairs(resolution):
    return [
...
class Version1Crosshair(Crosshair):
    ...
    def drawCrosshair(self,image,pixel_bullet_drop=0):
        ...
        draw.line((x1+thick_line_padding,y_midpoint,end_of_line_x-thick_line_length,y_midpoint),fill=main_color,width=4)
        for offset in [0,1]:
            draw.line((end_of_line_x+thick_line_padding+(offset*thick_line_padding*1.5),y_midpoint,
                end_of_line_x+(thick_line_padding*2)+(offset*thick_line_padding*1.5),y_midpoint),fill=main_color,width=2)

        #right lines
        start_of_line_x = x2-(diameter/3)
        draw.line((x2,y_midpoint,start_of_line_x,y_midpoint),fill=main_color,width=2)
        draw.line((x2-thick_line_padding,y_midpoint,start_of_line_x+thick_line_length,y_midpoint),fill=main_color,width=4)
        for offset in [0,1]:
            draw.line((start_of_line_x-thick_line_padding-(offset*thick_line_padding*1.5),y_midpoint,
                start_of_line_x-(thick_line_padding*2)-(offset*thick_line_padding*1.5),y_midpoint),fill=main_color,width=2)

        #moving part!
        #line
        tip_of_line_y = y_midpoint + pixel_bullet_drop
        draw.line((x_midpoint,tip_of_line_y,x_midpoint,y_midpoint+radius),fill='red',width=1)
        #triangle
        triangle_side_length = diameter / 50
        draw.line((x_midpoint,tip_of_line_y,x_midpoint-triangle_side_length,tip_of_line_y+triangle_side_length),fill='red',width=1)
        draw.line((x_midpoint,tip_of_line_y,x_midpoint+triangle_side_length,tip_of_line_y+triangle_side_length),fill='red',width=1)

        #bottom fixed line
        draw.line((x_midpoint,y2,x_midpoint,y2-thick_line_padding),fill=main_color,width=2)
        draw.line((x_midpoint,y2-thick_line_padding,x_midpoint,y2-(diameter/3)+thick_line_length),fill=main_color,width=4)
