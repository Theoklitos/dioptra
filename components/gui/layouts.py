...
class Layout1():
    ...
    def draw_layout(self,image,gui_text):
        ...
        #main info panel
        draw.text((5,25),'Magnification: {}'.format(self._format_magnification_text(gui_text['magnification'])),color='white')
        draw.text((5,45),'Range: ?',color='white')
        if(self.type != LayoutType.Standard):
            #fps/debug stuff
            draw.rectangle((resolution[0]-101,458,resolution[0],480),outline='black',fill='black')
            draw.text((resolution[0]-96,458),'FPS: {:d} [V{}]'.format(gui_text['fps'],gui_text['version']),color='white')
            #adjustment stuff
            draw.text((5,65),'Adjustment: {},{}'.format(1,2),color='white')
        #notifications
        if(self.notification is not None):
            length = len(self.notification) * 7
